PLS_v2_kfoldcv <- function(dataY,dataX,nt=2,limQ2set=.0975,modele="pls", family=NULL, K=nrow(dataX), NK=1, grouplist=NULL, random=FALSE, scaleX=TRUE, scaleY=NULL, keepcoeffs=FALSE, keepfolds=FALSE, keepdataY=TRUE, keepMclassed=FALSE, tol_Xi=10^(-12)) {
    res <- NULL
    res$nr <- nrow(dataX)
        if (K > res$nr) {
    ...
    for (nnkk in 1:NK) {
        ...
        for (ii in 1:K) {
            ...
            if (K == 1) {
                temptemp <- PLS_v2_wvc(dataY=dataY, dataX=dataX, nt=nt, dataPredictY=dataX, modele=modele,family=family,scaleX=scaleX,scaleY=scaleY,keepcoeffs=keepcoeffs,tol_Xi=tol_Xi)
                ...
                if (keepdataY) {dataY_kfolds[[nnkk]][[ii]] = NULL}
                  ...
                  cat(paste(ii,"\n"))
                  temptemp <- PLS_v2_wvc(dataY=dataY[-nofolds], dataX=dataX[-nofolds,], nt=nt, dataPredictY=dataX[nofolds,], modele=modele,family=family,scaleX=scaleX,scaleY=scaleY,keepcoeffs=keepcoeffs,tol_Xi=tol_Xi)
                  respls_kfolds[[nnkk]][[ii]] <- temptemp$valsPredict
